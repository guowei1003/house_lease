...
class Connection(object):
    ...
    def __init__(self, host, database, user=None, password=None,
        ...
        args = dict(conv=CONVERSIONS, use_unicode=True, charset="utf8",
                    db=database, init_command=('SET time_zone = "%s"' % time_zone), sql_mode="TRADITIONAL")
        if user is not None:
            args["user"] = user
    ...
    def reconnect(self):
        ...
        #     pool_con = PooledDB.PooledDB(creator=MySQLdb, mincached=10, maxcached=20, maxshared=100,
        #                                  maxconnections=200, blocking=False, maxusage=100, **self._db_args)
        ...
        try:
            ...
            logger.error("MySQL connect Error %s", str(e), exc_info=True)
            time.sleep(2)
            self._db = MySQLdb.connect(**self._db_args)
            self._db.autocommit(True)
        ...
        try:
            if self.mutex:
                if self.mutex.acquire(1):
                    x = cursor.execute(query, kwparameters or parameters)
                    return x
            else:
